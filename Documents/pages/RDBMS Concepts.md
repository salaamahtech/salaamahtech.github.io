	...
	- ## Tablespaces
	  collapsed:: true
		...
		- ### Default Tablespaces
			...
			- **2) System temporary space (TEMPSPACE1)**: is the work area for the database manager to perform operations, like joins and overflowed sorts. There must be at least one system temporary table space in each database.
			- **3) Default user table space (USERSPACE1)**: This is created by default, but you can delete it. 
		...
		- ### Types of namespaces
			- 1. **System-managed space (SMS)**
				- This type of table space is managed by the operating system and requires minimal administration. This is the default table space type.
			- 2. **Database-managed space (DMS)**
				- This type of table space is managed by the DB2 database manager, and it requires some administration
	- ## Buffer Pools
	  collapsed:: true
